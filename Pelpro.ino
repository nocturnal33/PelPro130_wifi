...
// (Make sure to fill these in with your specific network details)
IPAddress local_IP(192, 168, 1, 200); // Change this IP to a static IP on your network
IPAddress gateway(192, 168, 1, 1);    // Use your Local IP gateway - or leave blank to keep on LAN mode
...
void updateWiFiLED() {
  ...
  if (lastWifiStatus == WL_CONNECTED && wifiStatus != WL_CONNECTED) {
    ...
    // REMOVED: myServo.write(0) and HeatLevel = 0
    Serial.println("WiFi disconnected - Maintaining current Heat Level");
  }
  ...
  switch (wifiStatus) {
    case WL_CONNECTED:
      // Solid LED when connected (active-low LED usually, usually LOW is ON for built-in)
      // If your LED is off when connected, swap LOW/HIGH here
...
void setup() {
  ...
  myServo.write(0); // Start at 0 on POWER UP only
  ...
  // Configure static IP
  // IMPORTANT: Ensure your specific IP addresses are entered here
